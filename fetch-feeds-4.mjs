...
async function main() {
  ...
  for (const src of cfg.sources) {
    try {
      ...
      const items = normaliseItems(feed.items, src, feed.title, src.mode === "news" ? 10 : 15);
      trusted.push(...items);
      sourceStatus.push({ name: src.name, track: src.track, cat: src.cat, mode: src.mode, ok: true, count: items.length });
      console.log(`ok    [${src.track}/${src.cat}] ${src.name} (${items.length})`);
    } catch (e) {
      sourceStatus.push({ name: src.name, track: src.track, cat: src.cat, mode: src.mode, ok: false, count: 0 });
      console.log(`FAIL  [${src.track}/${src.cat}] ${src.name}: ${e.message}`);
    }
  }
  trusted = dedupe(trusted).sort(byDateDesc);

  // ---------- Discover (Google News topic searches) ----------
  let discover = [];
  for (const t of cfg.topics) {
    try {
      const feed = await parser.parseURL(gnUrl(t.query, loc));
      ...
      console.log(`ok    topic [${t.track}/${t.cat}]: ${t.query}`);
    } catch (e) {
      console.log(`FAIL  topic [${t.track}/${t.cat}] ${t.query}: ${e.message}`);
    }
  }
  discover = dedupe(discover).sort(byDateDesc);

  // ---------- Candidate publishers (not already on the list) ----------
  // Tracked per category so you know where a new publisher would slot in.
  ...
  const candidates = Object.entries(counts)
    ...
    .slice(0, 30)
    .map(([key, count]) => {
      const [name, track, cat] = key.split("||");
      return { name, track, cat, count };
    });
